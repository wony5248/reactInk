      ...
      <div ref={canvasRef} id="slide" style={{ width: "1200px", height: "1200px", border: "1px solid black" }}>
        <div style={{ width: "auto", height: "100%" }}>
          <svg width="100%" height="100%">
            {cmyk?(<g><path onClick={() => setPat2("0,0,0,0")} stroke={`rgb(${r},${g},${b})`} fill="transparent"  strokeLinecap="round" strokeWidth={widt} d={`M ${pat2}`}></path></g>):(<path onClick={() => setPat("0,0,0,0")} fill="transparent" stroke={`rgb(${r},${g},${b})`} strokeLinecap="round" strokeWidth="1" d={`M ${pat}`}></path>)}
            {inkresult.length ? (inkresult.map((item: any, index: Number) => { return <g className="isolate"><path onClick={(e) => { arrowClick(index, e) }} strokeLinecap="round" fill="transparent" stroke={`rgb(0,255,0)`} strokeWidth={"1"} d={`M ${item.join()}`}></path></g> })) : (null)}
            {inkresult2.map((items:any, index:any) => {return <g className="isolate">{items.map((item:any) => {return <path onClick={(e) => {console.log("hello")}} strokeLinecap="round" fill="transparent" stroke={`rgb(0,255,0)`} strokeWidth={item[4]} d={`M ${item[0]} ${item[1]} ${item[2]} ${item[3]}`}></path>})}</g>})}
